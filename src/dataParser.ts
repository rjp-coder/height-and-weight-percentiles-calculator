...
export function parseData(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    ignoreColumns.forEach((columnIndexToIgnore, i) => {
      if (columnIndexToIgnore < 0) {
        ignoreColumns[i] += partsWithMaybeExtraColumns.length;
      }
    });
    for (let j = 0; j < partsWithMaybeExtraColumns.length; j++) {
      if (
        ignoreColumns.includes(j) ||
        ignoreColumns.includes(j + partsWithMaybeExtraColumns.length)
      ) {
        continue; // Skip ignored columns
      }
      const part = partsWithMaybeExtraColumns[j];
      parts.push(part);
    }
